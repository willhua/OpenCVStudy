...
/************************************************************************/
void
LyhDehazor::MinFilter(unsigned char *data, int width, int height, int boxWidth, int boxHeight,
                      unsigned char *out) {
    //LOG("MinFilter   begin");

    unsigned char replaceMin = 255, reserveMin = 255; //保留部分和替换部分的最小值
    unsigned char *minForRow = (unsigned char *) malloc(
            sizeof(unsigned char) * height); //保存当前处理的列范围内，每行的的最小值
    if (minForRow == NULL) {
        return; //ERROR
    }
...
void LyhDehazor::Dehazor(unsigned char *imageDataRGBA, int width, int height) {
    ...
    if (DEBUG) {
        ...
        writeImgF(width, height, trans2, "refine-transmission.jpg");
    }
...
//out:输出A 的rgb值
void
LyhDehazor::AirlightEsimation(unsigned char *oriImage, unsigned char *oriDark, unsigned char *out,
                              int len, int low,
                              int heigh) {
...
void LyhDehazor::MeanFilter(T1 *data, T3 *outdata, int r, int width, int height, T2 cumtype) {
    ...
    for (int i = 0;
         i < (r + 1) * width; i += block)   //不需要担心end是不是block的整数倍，就算不是，超过的部分也会在后面重新计算正确的值
    {
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < r + 1; ++x) {
            ...
            outdata[y * width + x] =
                    cum[y * width + x + r] / mDivN[y * width + x]; //！！！！！！！！！mDivN[y * width + x]
...
void LyhDehazor::GuidedFilter(unsigned char *ori_r, unsigned char *ori_g, unsigned char *ori_b,
                              float *data, int width,
                              int height, int size, float e) {
    BoxDivN(mDivN, width, height, size);
    ...
    for (int i = 0; i < LEN; ++i) {
        ...
        float D = mrr[i] * (mgg[i] * mbb[i] - mbg[i] * mbg[i]) -
                  mrg[i] * (mrg[i] * mbb[i] - mrb[i] * mbg[i]) +
                  mrb[i] * (mrg[i] * mbg[i] - mrb[i] * mgg[i]);
        float v1 = (mgg[i]) * (mbb[i]) - mbg[i] * mbg[i];
        v1 /= D;
        float v2 = mbg[i] * mrb[i] - mrg[i] * (mbb[i]);
        v2 /= D;
        float v3 = (mrr[i]) * (mbb[i]) - mrb[i] * mrb[i];
        v3 /= D;
        float v4 = mrg[i] * mbg[i] - (mgg[i]) * mrb[i];
        v4 /= D;
        float v5 = mrg[i] * mrb[i] - (mrr[i]) * mbg[i];
        v5 /= D;
        float v6 = (mrr[i]) * (mgg[i]) - mrg[i] * mrg[i];
        v6 /= D;
