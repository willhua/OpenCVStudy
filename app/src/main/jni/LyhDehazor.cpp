...
/************************************************************************/
void
LyhDehazor::MinFilter(UCHAR *data, int width, int height, int boxWidth, int boxHeight, UCHAR *out) {
    LOG("MinFilter   begin");
...
//out:输出A 的rgb值
void LyhDehazor::AirlightEsimation(UCHAR *oriImage, UCHAR *oriDark, UCHAR *out, int len, int low,
                                   int heigh) {
...
void LyhDehazor::MeanFilter(T1 *data, T1 *outdata, int r, int width, int height, T2 cumtype) {
    ...
    for (int y = 0; y < height; ++y) {
        ...
        for (int x = r + 1; x < width - r; ++x) {
            outdata[y * height + x] =
                    (cum[y * height + x + r] - cum[y * height + x - r - 1]) / mDivN[y * height + x];
        }
...
void LyhDehazor::MulMeanFilter(T1 *data1, T2 *data2, T3 *outdata, int r, int width, int height) {
    ...
    for (int y = 0; y < height; ++y) {
        ...
        for (int x = width - r; x < width; ++x) {
            outdata[y * height + x] =
                    (cum[y * height + width - 1] - cum[y * height + x - r - 1]) / N;
        }
    }
...
void LyhDehazor::Normalized(UCHAR *data, float *out, int len)
{
    for(int i = 0; i < len; ++i)
    {
        out[i] = data[i] / 255.0f;
...
void LyhDehazor::GuidedFilter(UCHAR *ori_r, UCHAR *ori_g, UCHAR *ori_b, float *data, int width,
